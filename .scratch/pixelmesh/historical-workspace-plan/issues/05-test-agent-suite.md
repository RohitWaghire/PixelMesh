...
- [ ] Executable test runner script (`scripts/test-agent-client.ts`) demonstrating autonomous Ed25519 key generation, self-registration at `/api/auth/register`, header signing, and `/api/mcp` tool execution
...
- [ ] Verification of local server build (`npm run build` and `npm run dev`)
