...
- [x] Complete `@modelcontextprotocol/sdk` tool registration with strict schemas for all 21+ filters
- [x] `/api/mcp/route.ts` handling JSON-RPC requests (`tools/list`, `tools/call`, `resources/list`)
- [x] Cryptographic signature verification middleware on `/api/mcp` rejecting expired timestamps, replayed nonces, or invalid signatures
- [x] Credit metering: deduct 1 credit per filter, 3 credits per batch pipeline, and return `X-Agent-Credits-Remaining` header
- [x] Non-fatal MCP error contract: return `{ isError: true, content: [{ type: "text", text: "..." }] }` on parameter failures with zero credit penalty
