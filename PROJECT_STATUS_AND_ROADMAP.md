...
| **Anti-Replay Defense** | Distributed Sliding-Window Nonce Cache | ✅ Operational | `lib/auth/nonce-cache.ts` & `lib/redis/client.ts`: Distributed Redis cache verifying nonces within a 60-second rolling window with atomic `SET EX 60 NX`. |
| **Key Persistence & Keyring** | PostgreSQL + Prisma Relational Store | ✅ Operational | `lib/auth/key-store.ts` & `lib/db/prisma.ts`: Transaction-safe relational database with `AgentKey`, `CreditTransaction`, `AuditLog`, and `Organization` models. |
...
| **Task Queue & Async Worker** | Asynchronous Job Queue (BullMQ + Redis) | ✅ Operational | `lib/queue/job-queue.ts` & `lib/queue/worker.ts`: Priority scheduling (fast/default/batch), 3-tier concurrency limits, atomic credit billing upon job completion, `/api/mcp/jobs` polling and SSE streams. |
| **Object Storage Transport** | S3 / Cloudflare R2 + Dual-Mode Fallback | ✅ Operational | `lib/storage/client.ts` & `app/api/mcp/upload-url/route.ts`: Pre-signed PUT upload URLs, direct storage streams to Sharp, memory/local filesystem test adapters. |
